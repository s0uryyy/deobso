...
final class FolderImportScreen extends Screen {
    ...
    private void button(int x, int y, int width, String text, Runnable action, boolean enabled) {
        PopupButton button = new PopupButton(x, y, width, 20, Component.literal(text), b -> action.run(), java.util.function.Supplier::get);
    ...
    protected void init() {
        ...
        if (accounts == null) {
            folderPath = new PopupBox(this.font, left, 26, wide - 46, 20, null,
                    Component.literal("Folder path"), this::openPath, false);
