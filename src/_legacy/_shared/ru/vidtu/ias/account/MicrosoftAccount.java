...
public final class MicrosoftAccount implements Account {
    ...
    public void login(@NotNull LoginHandler handler, Runnable onComplete) {
        if (ru.vidtu.ias.auth.ely.ElyAuth.injectorAvailable()) {
            handler.error(new IllegalStateException("Restart without the Ely.by authlib-injector to use Microsoft accounts."));
            return;
        }
        try {
            // Skip if cancelled.
