...
public final class IASStorage {
    ...
    public static void disclaimers(@NotNull Path path) {
        try {
            ...
            // Get the path.
            path = AccountDirectory.prepare(path);
    ...
    public static void load(@NotNull Path path) {
        try {
            ...
            // Get the file.
            Path folder = AccountDirectory.prepare(path).resolve(".hidden");
    ...
    public static void save(@NotNull Path path) {
        try {
            ...
            // Get the file.
            Path file = AccountDirectory.prepare(path).resolve(".hidden/accounts_v1.do_not_send_to_anyone");
            ...
            try (ByteArrayOutputStream byteOut = new ByteArrayOutputStream();
                ...
                // Write the length.
                if (list.length > 65535) throw new java.io.IOException("Too many accounts");
            ...
            // Replace atomically so a failed import/save never truncates the previous storage.
            Path temporary = Files.createTempFile(file.getParent(), ".deobso-", ".tmp");
            try {
                Files.write(temporary, data, StandardOpenOption.TRUNCATE_EXISTING,
                        StandardOpenOption.WRITE, StandardOpenOption.SYNC, LinkOption.NOFOLLOW_LINKS);
                try {
    ...
    public static void gameDisclaimerShown(@NotNull Path path) {
        try {
            ...
            // Create the file.
            Path file = AccountDirectory.prepare(path).resolve(".hidden/game_disclaimer_shown");
            Files.createDirectories(file.getParent());
