...
public final class OfflinePopupScreen extends Screen implements LastPassRenderCallback {
    ...
    protected void init() {
        ...
        this.customUuid = new PopupBox(this.font, this.width / 2 - 75, this.height / 2 + 19, 148, 20,
                this.customUuid, new net.minecraft.network.chat.TextComponent("UUID (Ctrl: force)"), this::done, false, new net.minecraft.network.chat.TextComponent("UUID (Ctrl: force)"));
        this.customUuid.setMaxLength(128);
        this.addRenderableWidget(this.customUuid);
        this.customUuid.setResponder(value -> this.type(false));

        // Add done button.
        this.doneTooltip = new LegacyTooltip(this, this.font, null, -1);
        this.done = new PopupButton(this.width / 2 - 75, this.height / 2 + 43, 74, 20,
        ...
        // Add cancel button.
        PopupButton button = new PopupButton(this.width / 2 + 1, this.height / 2 + 43, 74, 20,
    ...
    private void done() {
        ...
        // Prevent NPE.
        if (this.name == null || this.customUuid == null || this.locked || this.cancelled) return;
        this.type(false);
        if (this.done == null || !this.done.active) return;
        final UUID chosenUuid = ru.vidtu.ias.account.CustomUuid.resolve(
                this.name.getValue(), this.customUuid.getValue(), controlDown());
        ...
        if (length < 3 || length > 16) {
            ...
            this.handler.accept(new OfflineAccount(value, chosenUuid, null));
        ...
        MSAuth.nameToMcp(value).whenCompleteAsync((profile, throwable) -> {
            ...
            this.handler.accept(new OfflineAccount(value, chosenUuid, skin));
        }, this.minecraft);
    }
    ...
    private void type(boolean instant) {
        ...
        // Validate UUID before enabling submission; Ctrl explicitly opts into conversion.
        if (this.customUuid != null && !ru.vidtu.ias.account.CustomUuid.valid(this.customUuid.getValue()) && !controlDown()) {
            this.doneTooltip.tooltip(new net.minecraft.network.chat.TextComponent("Invalid UUID. Hold Ctrl to convert to: " + ru.vidtu.ias.account.CustomUuid.resolve(this.name.getValue(), this.customUuid.getValue(), true)));
            this.done.active = false;
    ...
    public void tick() {
        super.tick();
        this.name.tick();
        if (this.customUuid != null) this.customUuid.tick();
        this.type(false);
    }
    ...
    public boolean keyPressed(int key, int scan, int mods) {
        ...
        if (key == GLFW.GLFW_KEY_LEFT_ALT || key == GLFW.GLFW_KEY_RIGHT_ALT || key == GLFW.GLFW_KEY_LEFT_CONTROL || key == GLFW.GLFW_KEY_RIGHT_CONTROL) {
            this.type(false);
        }
        return res;
    }
    ...
    public void renderBackground(PoseStack pose) {
        ...
        fill(pose, centerX - 80, centerY - 50, centerX + 80, centerY + 68, 0xF8_20_20_30);
        fill(pose, centerX - 79, centerY - 51, centerX + 79, centerY - 50, 0xF8_20_20_30);
        fill(pose, centerX - 79, centerY + 68, centerX + 79, centerY + 69, 0xF8_20_20_30);
    }
    ...
    private boolean controlDown() {
        ...
        return window != 0 && (GLFW.glfwGetKey(window, GLFW.GLFW_KEY_LEFT_CONTROL) == GLFW.GLFW_PRESS
