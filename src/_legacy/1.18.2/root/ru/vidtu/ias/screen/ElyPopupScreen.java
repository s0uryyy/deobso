...
final class ElyPopupScreen extends Screen {
    ...
    ElyPopupScreen(Screen parent, Consumer<Account> handler) {
        super(new net.minecraft.network.chat.TextComponent("Ely.by"));
        this.parent = parent;
    ...
    private PopupBox field(int y, String title, PopupBox previous, boolean secret) {
        PopupBox box = new PopupBox(this.font, this.width / 2 - 140, y, 280, 20,
                previous, new net.minecraft.network.chat.TextComponent(title), this::submit, secret, new net.minecraft.network.chat.TextComponent(title));
        box.setMaxLength(secret ? 1024 : 254);
        box.active = !busy;
        if (secret) box.setFormatter((s, i) -> net.minecraft.util.FormattedCharSequence.forward(
    ...
    private void label(int y, String text) {
        PopupButton label = new PopupButton(this.width / 2 - 150, y, 300, 20,
                new net.minecraft.network.chat.TextComponent(text), b -> {}, ru.vidtu.ias.legacy.LegacyTooltip.EMPTY);
        label.active = false;
    ...
    @Override protected void init() {
        ...
        PopupButton submit = new PopupButton(this.width / 2 - 140, top + 160, 280, 20,
                new net.minecraft.network.chat.TextComponent("Add Ely.by account"), b -> submit(), ru.vidtu.ias.legacy.LegacyTooltip.EMPTY);
        ...
        this.addRenderableWidget(new PopupButton(this.width / 2 - 140, top + 184, 280, 20,
                new net.minecraft.network.chat.TextComponent("Cancel"), b -> onClose(), ru.vidtu.ias.legacy.LegacyTooltip.EMPTY));
    }
    private void submit() {
