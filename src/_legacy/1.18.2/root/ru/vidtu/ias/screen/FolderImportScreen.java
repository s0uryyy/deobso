...
final class FolderImportScreen extends Screen {
    ...
    FolderImportScreen(Screen parent) {
        super(new net.minecraft.network.chat.TextComponent("Add accounts from folder"));
        this.parent = parent;
    }

    private void button(int x, int y, int width, String text, Runnable action, boolean enabled) {
        PopupButton button = new PopupButton(x, y, width, 20, new net.minecraft.network.chat.TextComponent(text), b -> action.run(), ru.vidtu.ias.legacy.LegacyTooltip.EMPTY);
    ...
    protected void init() {
        ...
        if (accounts == null) {
            folderPath = new PopupBox(this.font, left, 26, wide - 46, 20, null,
                    new net.minecraft.network.chat.TextComponent("Folder path"), this::openPath, false, new net.minecraft.network.chat.TextComponent("Folder path"));
