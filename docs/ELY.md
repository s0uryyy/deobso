...
Первые CI-запросы к repo.llaun.ch вернули HTTP 403. В запуске
https://github.com/s0uryyy/deobso/actions/runs/34970060016 запрос с явным User-Agent
успешен: SHA1 точного JAR 7.0.61-ely.1 и поле ElyProfileService проверены.
Все шесть Fabric-сборок и core-тесты прошли. Итог — один общий ZIP-артефакт.
Проверка структуры библиотеки не заменяет ручной вход/проверку скинов в игре.
При будущем HTTP 403 проверка будет явно отмечена как пропущенная.
